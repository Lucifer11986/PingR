// fix-identity-migration.js
// Fixt Conversations ohne identityParticipants

const mongoose = require('mongoose');
require('dotenv').config({ path: '/opt/pingr/.env' });
...
async function run() {
  try {
    // Verbinde zu MongoDB (aus .env MONGO_URL)
    const mongoUrl = process.env.MONGO_URL || 'mongodb://mongo:27017/pingr';
    await mongoose.connect(mongoUrl);
    console.log('✅ Connected to MongoDB');

    // Finde alle Conversations ohne identityParticipants
    ...
    for (const conv of conversations) {
      try {
        ...
        // Finde passende Identities zu den Usern
        const identities = await Identity.find({
          userId: { $in: conv.participants }
        }).select('_id userId username');

        if (!identities.length) {
          console.warn(`   ⚠️ No identities found for conversation ${conv._id}`);
          skipped++;
          continue;
        }
        ...
        identities.forEach(i => {
          console.log(`     - ${i._id} (User: ${i.userId}, Name: ${i.username || 'unnamed'})`);
        });

        // Setze identityParticipants
        conv.identityParticipants = identities.map(i => i._id);
        await conv.save();

        console.log(`   ✅ Fixed! Set identityParticipants: [${conv.identityParticipants.join(', ')}]`);
        fixed++;

      } catch (err) {
        console.error(`   ❌ Error fixing ${conv._id}:`, err.message);
        skipped++;
      }
    }
