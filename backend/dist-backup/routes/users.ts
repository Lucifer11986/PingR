...
router.patch('/avatar', upload.single('avatar'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/me/export', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await User.findById(userId).select('-password -resetToken -emailVerifyToken').lean()
    if (!user) { res.status(404).json({ error: 'Nicht gefunden' }); return }
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="PingR_Datenauszug_${(user as any).username}_${new Date().toISOString().slice(0,10)}.json"`
    )
    res.json(exportData)
  } catch (err) {
