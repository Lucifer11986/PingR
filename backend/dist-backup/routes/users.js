...
router.get('/search', async (req, res) => {
    try {
        ...
        const currentUser = await User_1.User.findById(req.userId).select('blockedUsers');
...
router.get('/:id/profile', async (req, res) => {
    try {
        const user = await User_1.User.findById(req.params.id)
            .select('-password -resetToken -resetTokenExpires -lastIP -blockedUsers');
        if (!user) {
            res.status(404).json({ error: 'Nutzer nicht gefunden' });
            return;
        }
        const sharedGroups = await Conversation_1.Conversation.find({
            isGroup: true,
...
router.patch('/status', async (req, res) => {
    try {
        ...
        if (statusExpiresAt)
            update.statusExpiresAt = new Date(statusExpiresAt);
        const user = await User_1.User.findByIdAndUpdate(req.userId, update, { new: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        (0, socketServer_1.getIO)().emit('user_status', { userId: req.userId, status: user.status, lastSeen: user.lastSeen });
        res.json(user);
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.patch('/privacy', async (req, res) => {
    try {
        ...
        const update = {};
        if (privacyShowStatus)
            update.privacyShowStatus = privacyShowStatus;
        if (privacyShowLastSeen)
            update.privacyShowLastSeen = privacyShowLastSeen;
        if (privacyShowAvatar)
            update.privacyShowAvatar = privacyShowAvatar;
        const user = await User_1.User.findByIdAndUpdate(req.userId, update, { new: true }).select('-password');
        res.json(user);
    }
    catch (_err) {
...
router.patch('/profile', async (req, res) => {
    try {
        const { bio } = req.body;
        const user = await User_1.User.findByIdAndUpdate(req.userId, { bio: bio?.substring(0, 200) || '' }, { new: true }).select('-password');
        res.json(user);
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.patch('/avatar', upload_1.upload.single('avatar'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const user = await User_1.User.findByIdAndUpdate(req.userId, { avatar: `/uploads/${req.file.filename}` }, { new: true }).select('-password');
        res.json(user);
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.post('/:id/block', async (req, res) => {
    try {
        const targetId = new mongoose_1.Types.ObjectId(req.params.id);
        await User_1.User.findByIdAndUpdate(req.userId, { $addToSet: { blockedUsers: targetId } });
...
router.delete('/:id/block', async (req, res) => {
    try {
        const targetId = new mongoose_1.Types.ObjectId(req.params.id);
        await User_1.User.findByIdAndUpdate(req.userId, { $pull: { blockedUsers: targetId } });
...
router.post('/bookmarks/:messageId', async (req, res) => {
    try {
        const msgId = new mongoose_1.Types.ObjectId(req.params.messageId);
        const user = await User_1.User.findById(req.userId).select('bookmarks');
        if (!user) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        ...
        if (isBookmarked) {
            await User_1.User.findByIdAndUpdate(req.userId, { $pull: { bookmarks: msgId } });
            res.json({ ok: true, bookmarked: false });
        }
        else {
            await User_1.User.findByIdAndUpdate(req.userId, { $addToSet: { bookmarks: msgId } });
...
router.post('/scheduled', async (req, res) => {
    try {
        ...
        const msg = await ScheduledMessage_1.ScheduledMessage.create({
...
router.get('/scheduled', async (req, res) => {
    try {
        const msgs = await ScheduledMessage_1.ScheduledMessage.find({ sender: req.userId, sent: false })
...
router.delete('/scheduled/:id', async (req, res) => {
    try {
        await ScheduledMessage_1.ScheduledMessage.findOneAndDelete({ _id: req.params.id, sender: req.userId });
        res.json({ ok: true });
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.get('/:id/lastseen', async (req, res) => {
    try {
        const user = await User_1.User.findById(req.params.id)
...
router.post('/e2e-key', async (req, res) => {
    try {
        const { publicKey } = req.body;
        if (!publicKey) {
            res.status(400).json({ error: 'publicKey fehlt' });
            return;
        }
        await User_1.User.findByIdAndUpdate(req.userId, { e2ePublicKey: publicKey });
        res.json({ ok: true });
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.get('/:id/e2e-key', async (req, res) => {
    try {
        const user = await User_1.User.findById(req.params.id).select('e2ePublicKey username');
        if (!user) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
...
router.delete('/me', async (req, res) => {
    try {
        ...
        for (const msg of userMessages) {
            if (msg.fileUrl) {
                try {
                    const fp = path_1.default.join('/app/uploads', path_1.default.basename(msg.fileUrl));
        ...
        await Message.updateMany({ sender: userId }, { deleted: true, content: '[Nutzer gelöscht]', fileUrl: null, fileName: null });
        // 2. Profilbild löschen
        const user = await User_1.User.findById(userId);
        if (user && user.avatar) {
            try {
                const fp = path_1.default.join('/app/uploads', path_1.default.basename(user.avatar));
                if (fs_1.default.existsSync(fp))
                    fs_1.default.unlinkSync(fp);
            }
            catch (_e) { }
        ...
        await User_1.User.updateMany({ blockedUsers: userId }, { $pull: { blockedUsers: userId } });
...
router.get('/me/export', async (req, res) => {
    try {
        const userId = req.userId;
        const user = await User_1.User.findById(userId).select('-password -resetToken -emailVerifyToken').lean();
        if (!user) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        ...
        const exportData = {
            exportInfo: {
                ...
                dataController: 'PingR Messenger — lumestack.de',
            },
            personalData: {
                uin: user.uin,
                username: user.username,
        ...
        res.setHeader('Content-Disposition', `attachment; filename="PingR_Datenauszug_${user.username}_${new Date().toISOString().slice(0, 10)}.json"`);
        res.json(exportData);
    }
    catch (err) {
...
router.get('/availability', async (req, res) => {
    try {
        const user = await User_1.User.findById(req.userId).select('availability timezone');
        res.json({
            availability: user?.availability || { enabled: false, days: [1, 2, 3, 4, 5], startTime: '09:00', endTime: '22:00', message: '' },
...
router.patch('/availability', async (req, res) => {
    try {
        ...
        await User_1.User.findByIdAndUpdate(req.userId, update);
        res.json({ ok: true });
    }
    catch (_err) {
        res.status(500).json({ error: 'Serverfehler' });
...
router.get('/:id/availability', async (req, res) => {
    try {
        const user = await User_1.User.findById(req.params.id).select('availability timezone username');
        if (!user) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        ...
        if (!avail?.enabled) {
            res.json({ available: true, timezone, availability: null });
            return;
        }
