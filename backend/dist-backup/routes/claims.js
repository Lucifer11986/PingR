...
const LegacyClaim_1 = require("../models/LegacyClaim");
...
async function sendClaimEmail(email, username, uin, token) {
...
router.get('/mine', async (req, res) => {
    try {
        const claims = await LegacyClaim_1.LegacyClaim.find({ userId: req.userId })
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!uin || !isValidUIN(String(uin))) {
            res.status(400).json({ error: 'Ungültige UIN (5-10 Ziffern)' });
            return;
        }
        if (!['email', 'firstcome'].includes(proofType)) {
            res.status(400).json({ error: 'Ungültiger Proof-Typ' });
            return;
        }
        const cleanUIN = String(uin).trim();
        // 1. Prüfen ob die UIN bereits als PingR-UIN vergeben ist
        const existingUser = await User_1.User.findOne({ uin: cleanUIN });
        if (existingUser && String(existingUser._id) !== req.userId) {
            res.status(409).json({ error: `UIN #${cleanUIN} ist bereits als PingR-Nummer vergeben.` });
            return;
        }
        // 2. Prüfen ob ein verifizierter Claim existiert
        const verifiedClaim = await LegacyClaim_1.LegacyClaim.findOne({ requestedUin: cleanUIN, status: 'approved' });
        if (verifiedClaim && String(verifiedClaim.userId) !== req.userId) {
            res.status(409).json({ error: `UIN #${cleanUIN} wurde bereits von einem anderen Nutzer verifiziert.` });
            return;
        }
        // 3. Prüfen ob dieser User schon einen aktiven Claim hat
        const myActiveClaim = await LegacyClaim_1.LegacyClaim.findOne({
            userId: req.userId,
            status: { $in: ['pending', 'email_sent', 'approved'] }
        });
        if (myActiveClaim) {
            res.status(409).json({ error: 'Du hast bereits einen aktiven Claim. Schließe ihn erst ab.' });
            return;
        }
        // 4. Prüfen ob UIN bereits beansprucht wird (pending)
        const pendingClaim = await LegacyClaim_1.LegacyClaim.findOne({
            requestedUin: cleanUIN,
            status: { $in: ['pending', 'email_sent'] }
        });
        if (pendingClaim && String(pendingClaim.userId) !== req.userId) {
            res.status(409).json({ error: `UIN #${cleanUIN} wird gerade von einem anderen Nutzer beansprucht.` });
            return;
        }
        if (proofType === 'email') {
            ...
            const claim = await LegacyClaim_1.LegacyClaim.create({
            ...
            await sendClaimEmail(proofEmail, user?.username || 'Nutzer', cleanUIN, token);
...
router.post('/screenshot', upload.single('screenshot'), async (req, res) => {
    try {
        const { uin } = req.body;
        if (!uin || !isValidUIN(String(uin))) {
            res.status(400).json({ error: 'Ungültige UIN' });
            return;
        }
        if (!req.file) {
            res.status(400).json({ error: 'Screenshot erforderlich' });
            return;
        }
        const cleanUIN = String(uin).trim();
        // Prüfungen wie oben
        const verifiedClaim = await LegacyClaim_1.LegacyClaim.findOne({ requestedUin: cleanUIN, status: 'approved', proofType: { $ne: 'firstcome' } });
        if (verifiedClaim && String(verifiedClaim.userId) !== req.userId) {
            res.status(409).json({ error: `UIN #${cleanUIN} wurde bereits verifiziert.` });
            return;
        }
        const myActiveClaim = await LegacyClaim_1.LegacyClaim.findOne({ userId: req.userId, status: { $in: ['pending', 'email_sent'] } });
        if (myActiveClaim) {
            res.status(409).json({ error: 'Du hast bereits einen ausstehenden Claim.' });
            return;
        }
        const claim = await LegacyClaim_1.LegacyClaim.create({
            userId: req.userId, requestedUin: cleanUIN,
...
router.get('/verify-email', async (req, res) => {
    try {
        ...
        const claim = await LegacyClaim_1.LegacyClaim.findOne({
            emailToken: token,
        ...
        const username = claim.userId?.username || 'Nutzer';
...
router.post('/:id/assign', async (req, res) => {
    try {
        const claim = await LegacyClaim_1.LegacyClaim.findById(req.params.id);
        if (!claim) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        if (claim.status !== 'approved') {
            res.status(400).json({ error: 'Claim nicht genehmigt' });
            return;
        }
        // UIN tauschen
        await User_1.User.findByIdAndUpdate(claim.userId, {
...
router.delete('/:id', async (req, res) => {
    try {
        const claim = await LegacyClaim_1.LegacyClaim.findOne({ _id: req.params.id, userId: req.userId });
        if (!claim) {
            res.status(404).json({ error: 'Nicht gefunden' });
            return;
        }
        if (claim.status === 'approved') {
            ...
            if (user) {
                const updateData = { legacyUin: undefined, legacyVerified: false, legacyMethod: undefined, legacyReserved: false };
                ...
                await User_1.User.findByIdAndUpdate(req.userId, updateData);
            }
        }
