...
router.post('/register', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (String(password).length < 6) { res.status(400).json({ error: 'Passwort min. 6 Zeichen' }); return }
    ...
    sendVerificationEmail(user.email, user.username, verifyToken, user.uin).then(sent => {
      if (!sent) console.log(`[Register] No SMTP configured — verify token: ${verifyToken}`)
    })
...
router.post('/login', checkIPBlacklist, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if ((user as any).twoFactorEnabled && (user as any).twoFactorSecret) {
      ...
      if (!verifyTOTP(String((user as any).twoFactorSecret), String(twoFaCode))) {
...
router.get('/me', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = await User.findById(req.userId).select('-password -twoFactorSecret -twoFactorBackup')
  ...
  try {
    ...
    try {
      ...
      await Promise.all([
        ...
        Message.updateMany(
          { sender: uid },
          { deleted: true, content: '', fileUrl: undefined, fileName: undefined }
        ),
...
router.delete('/account/cleanup-deleted', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
