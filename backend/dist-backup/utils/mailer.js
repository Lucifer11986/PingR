...
exports.sendReportAlert = sendReportAlert;
...
async function sendAdminAlert(subject, html) {
    ...
    try {
        ...
        await transporter.sendMail({
            from: `"PingR Security" <${process.env.SMTP_USER}>`,
            to: adminEmail,
            subject: `[PingR] ${subject}`,
            html,
        });
    }
    catch (err) {
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
      ...
      <div style="background:#1a1a2e;color:#f1f0f8;padding:24px;border-radius:0 0 8px 8px">
        ...
        <div style="margin-top:20px;padding:16px;background:#0d0d14;border-radius:8px">
          ...
          <a href="http://${process.env.SERVER_IP || '217.154.165.25'}:8084/admin"
             style="color:#60a5fa;text-decoration:none;font-size:14px">
            http://${process.env.SERVER_IP || '217.154.165.25'}:8084/admin
          </a>
        </div>
      </div>
    </div>
...
async function sendReportAlert(data) {
    const html = `
...
async function sendVerificationEmail(email, username, token, uin) {
    if (!isSmtpConfigured()) {
    ...
    try {
      ...
      <div style="background:linear-gradient(135deg,rgba(180,83,9,0.12),rgba(245,158,11,0.08));border:1px solid rgba(245,158,11,0.25);border-radius:14px;padding:18px 22px;margin-bottom:26px;text-align:center">
        <p style="margin:0 0 8px;color:rgba(255,255,255,0.4);font-size:11px;text-transform:uppercase;letter-spacing:0.1em;font-weight:700">🎯 Deine persönliche PingR-Nummer</p>
        ...
        await transporter.sendMail({
            from: `"PingR" <${process.env.SMTP_USER}>`,
            to: email,
            subject: 'PingR – Willkommen! Bitte E-Mail bestätigen',
            html: `<!DOCTYPE html>
<html lang="de">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Willkommen bei PingR</title></head>
...
<div style="max-width:540px;margin:0 auto;padding:40px 20px">
  ...
  <div style="background:linear-gradient(135deg,#92400e 0%,#b45309 40%,#d97706 70%,#f59e0b 100%);border-radius:20px 20px 0 0;padding:36px 32px;text-align:center;position:relative;overflow:hidden">
    ...
    <h1 style="margin:0;color:white;font-size:30px;font-weight:900;letter-spacing:-0.03em;position:relative">PingR</h1>
  ...
  <div style="background:#0d0f18;border:1px solid rgba(255,255,255,0.08);border-top:none;border-radius:0 0 20px 20px;padding:36px 32px">
    ...
    <p style="margin:0 0 6px;color:#d97706;font-size:13px;font-weight:600">Herzlich willkommen in der PingR-Community!</p>
    <p style="margin:0 0 28px;color:rgba(255,255,255,0.45);font-size:14px;line-height:1.7">
      Wir freuen uns riesig, dich dabei zu haben. Dein Konto wurde erfolgreich erstellt und wartet nur noch auf deine Bestätigung. Danach hast du Zugriff auf alle Funktionen — von verschlüsselten Nachrichten bis hin zu Gruppenchats.
    </p>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:14px;padding:18px 20px;margin-bottom:26px">
      <p style="margin:0 0 12px;color:rgba(255,255,255,0.5);font-size:11px;text-transform:uppercase;letter-spacing:0.1em;font-weight:700">Was dich bei PingR erwartet</p>
      <div style="display:flex;flex-direction:column;gap:8px">
        <div style="display:flex;align-items:center;gap:10px;font-size:13px;color:rgba(255,255,255,0.65)"><span style="font-size:16px">🔒</span> Ende-zu-Ende Verschlüsselung</div>
    ...
    <div style="border-top:1px solid rgba(255,255,255,0.06);padding-top:20px">
      <p style="color:rgba(255,255,255,0.25);font-size:12px;margin:0;line-height:1.7">
        ...
        Falls du dich nicht bei PingR registriert hast, ignoriere diese E-Mail einfach — deine Adresse wird nicht ohne Bestätigung verwendet.
      </p>
    </div>
...
async function sendPasswordResetEmail(email, username, code) {
    if (!isSmtpConfigured()) {
        console.log(`[Mailer] SMTP nicht konfiguriert — Reset-Code für ${username}: ${code}`);
        return false;
    }
    try {
        const transporter = createTransport();
        await transporter.sendMail({
            from: `"PingR" <${process.env.SMTP_USER}>`,
            to: email,
            subject: 'PingR – Passwort zurücksetzen',
            html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#0d0d14;color:#f1f0f8;padding:32px;border-radius:16px">
          <h1 style="font-size:28px;margin:0 0 28px">💬 PingR</h1>
...
async function sendLoginAlert(data) {
    if (!isSmtpConfigured())
        return false;
    try {
        const transporter = createTransport();
        await transporter.sendMail({
            from: `"PingR Security" <${process.env.SMTP_USER}>`,
            to: data.email,
            subject: 'PingR – Neuer Login erkannt',
            html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#0d0d14;color:#f1f0f8;padding:32px;border-radius:16px">
          <h1 style="font-size:24px;margin:0 0 4px">💬 PingR</h1>
