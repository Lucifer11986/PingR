...
function fuzzyAny(text: string, terms: string[]): string | null {
  const norm = normalize(text)
  for (const t of terms) {
    if (norm.includes(normalize(t))) return t
  }
  return null
}

// ── Heil-Hitler-Detektor (dediziert, alle Varianten) ─────────────────────────
function hasNaziGreeting(text: string): boolean {
  const norm    = normalize(text)
  const rawLow  = text.toLowerCase()

  // 1. Normalisiert-kombiniert
  if (norm.includes('heilhitler'))  return true
  if (norm.includes('hitlerheil'))  return true
  if (norm.includes('siegheil'))    return true
  if (norm.includes('sigheil'))     return true
  if (norm.includes('heilderfuhrer') || norm.includes('heildenfuhrer')) return true

  // 2. heil + fuhrer/führer (normalisiert)
  if (norm.includes('heil') && (norm.includes('fuhrer') || norm.includes('fhrer'))) return true

  // 3. heil + hitler-Varianten (auch teilweise)
  if (norm.includes('heil') && (
    norm.includes('hitl') || norm.includes('htler') || norm.includes('itler')
  )) return true

  // 4. Regex für original Text (Leerzeichen erlaubt)
  if (/heil\s+h(itler?|tler|!|i|1)?(\s|$|\.)/i.test(rawLow))    return true
  if (/heil\s+(h?itler|htler|itler)/i.test(rawLow))              return true
  if (/heil\s+(dem\s+)?(f[uü]h?rer?|fuhrer)/i.test(rawLow))     return true
  if (/s(ie)?g\s+heil/i.test(rawLow))                            return true
  if (/h\s*e\s*i\s*l\s+h\s*i\s*t\s*l\s*e\s*r/i.test(rawLow))  return true
...
function has88(text: string): boolean {
...
function isSpam(text: string): boolean {
  ...
  if (/(https?:\/\/\S+\s*){6,}/.test(text)) return true
  return false
}

// ── HAUPT-EXPORT ─────────────────────────────────────────────────────────────
export function filterContent(text: string): FilterResult {
  if (!text?.trim()) return { blocked: false, flagged: false }

  if (fuzzyAny(text, CSAM)) return {
  ...
  if (hasNaziGreeting(text)) return {
  ...
  if (has88(text)) return {
    blocked: true, flagged: false, category: 'extremism_certain', severity: 'critical',
    reason: 'Dieser Inhalt enthält nach § 86a StGB verbotenes Material.',
    legalBasis: '§ 86a StGB',
  }

  if (fuzzyAny(text, EXTREMISM)) return {
    blocked: true, flagged: false, category: 'extremism_certain', severity: 'critical',
    reason: 'Dieser Inhalt enthält nach § 86a StGB verbotenes Material.',
    legalBasis: '§ 86a StGB',
  }

  if (fuzzyAny(text, TERROR)) return {
  ...
  if (fuzzyAny(text, HARASSMENT)) return {
  ...
  if (fuzzyAny(text, SUSPICIOUS)) return {
...
export function filterFilename(filename: string): FilterResult {
...
export function filterGroupName(name: string): FilterResult {
  if (!name?.trim() || name.trim().length < 2)
    return { blocked: true, flagged: false, reason: 'Name zu kurz.', category: 'spam', severity: 'low' }
  const r = filterContent(name)
  if (r.blocked) return { ...r, reason: `Gruppenname nicht erlaubt: ${r.reason}` }
  return { blocked: false, flagged: false }
}