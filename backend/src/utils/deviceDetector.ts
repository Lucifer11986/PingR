...
export function detectDevice(userAgent: string): DeviceInfo {
  ...
  if (/tablet|ipad/i.test(ua)) {
    ...
    /mobile|android|iphone|ipod|blackberry|opera mini|windows phone|webos/i.test(ua) ||
  ...
  if (/edg\//i.test(ua)) {
    browser = 'Edge'
    const m = ua.match(/edg\/([\d.]+)/i); browserVersion = m?.[1] || ''
  } else if (/opr\//i.test(ua)) {
    browser = 'Opera'
    const m = ua.match(/opr\/([\d.]+)/i); browserVersion = m?.[1] || ''
  } else if (/firefox\//i.test(ua)) {
    browser = 'Firefox'
    const m = ua.match(/firefox\/([\d.]+)/i); browserVersion = m?.[1] || ''
  } else if (/samsungbrowser/i.test(ua)) {
    browser = 'Samsung Browser'
    const m = ua.match(/samsungbrowser\/([\d.]+)/i); browserVersion = m?.[1] || ''
  } else if (/chrome\//i.test(ua)) {
    browser = 'Chrome'
    const m = ua.match(/chrome\/([\d.]+)/i); browserVersion = m?.[1] || ''
  } else if (/safari\//i.test(ua)) {
    browser = 'Safari'
    const m = ua.match(/version\/([\d.]+)/i); browserVersion = m?.[1] || ''
...
export function formatDeviceForLog(device: DeviceInfo): string {
