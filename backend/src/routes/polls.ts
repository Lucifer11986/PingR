...
router.post('/', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!question?.trim() || !options || options.length < 2) {
      res.status(400).json({ error: 'Frage und mind. 2 Optionen erforderlich' }); return
    }
    if (options.length > 10) {
      res.status(400).json({ error: 'Max. 10 Optionen' }); return
    }
    const poll = await Poll.create({
      conversationId,
      createdBy: req.userId,
      question:  question.trim(),
      options:   options.map((o: string) => ({ text: o.trim(), votes: [] })),
...
router.get('/conversation/:convId', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const polls = await Poll.find({ conversationId: req.params.convId })
...
router.get('/:id', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!poll) { res.status(404).json({ error: 'Nicht gefunden' }); return }
    res.json(poll)
  } catch (_err) { res.status(500).json({ error: 'Serverfehler' }) }
})
...
router.post('/:id/vote', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (poll.endsAt && poll.endsAt < new Date()) {
      res.status(400).json({ error: 'Umfrage beendet' }); return
    }

    const userId = new Types.ObjectId(req.userId)
    const indexes: number[] = Array.isArray(optionIndexes) ? optionIndexes : [optionIndexes]
