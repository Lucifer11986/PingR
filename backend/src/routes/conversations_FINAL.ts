...
router.patch('/:id/avatar', upload.single('avatar'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/my-channels', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const conversations = await Conversation.find({
      isGroup: true,
      admins: userId,
    }).select('_id groupName groupAvatar isPublic');
